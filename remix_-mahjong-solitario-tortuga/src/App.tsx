...
export default function App() {
  ...
  const [playerCount, setPlayerCount] = useState(REQUIRED_PLAYERS);
  ...
  useEffect(() => {
    ...
    socket.on('room:joined', (payload: RoomJoinedPayload) => {
      ...
      setCurrentPlayers(payload.gameState.players.filter((player) => player.isConnected).length);
      setLobbyError('');
      setGameState('lobby');
    });

    socket.on('game:state', (state: SocketGameState) => {
      const connected = state.players.filter((player) => player.isConnected).length;
      setCurrentPlayers(connected);
      setPlayerCount(state.requiredPlayers);

      if (state.hasStarted) {
        setGameState((previous) => {
          if (previous === 'lobby' || previous === 'matchmaking') {
            return 'loading';
          }
          return previous;
        });
    ...
    return (
      <div className={`min-h-screen flex items-center justify-center p-6 transition-colors duration-500 ${isDarkMode ? 'bg-stone-950 text-white' : 'bg-stone-100 text-stone-900'}`}>
        ...
        >
          ...
          {matchmakingMode === 'create' ? (
            <div className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <div className={`px-6 py-4 rounded-2xl border-2 font-black text-center ${
                  isDarkMode ? 'bg-stone-800 border-stone-700 text-emerald-400' : 'bg-stone-50 border-stone-200 text-emerald-600'
                }`}>
                  4 jugadores (fijo)
                </div>
              </div>
            ...
            <div className="space-y-6">
              <div className="space-y-2 text-center">
                <p className={`text-sm ${isDarkMode ? 'text-stone-400' : 'text-stone-500'}`}>Ingresa el ID completo de la sala que te compartieron</p>
                <div className="flex justify-center gap-2">
                  <input 
                    ...
                    className={`w-full max-w-[360px] px-6 py-5 rounded-2xl border-2 outline-none transition-all font-black text-base text-center ${
                      isDarkMode ? 'bg-stone-800 border-stone-700 text-white focus:border-emerald-500/50' : 'bg-stone-50 border-stone-200 text-stone-900 focus:border-emerald-500'
                    }`}
                  />
                </div>
          ...
          {lobbyError && (
            <p className="text-sm text-red-400 text-center font-semibold">{lobbyError}</p>
          )}
        </motion.div>
      </div>
    );
        ...
        >
          <div className="space-y-2">
            ...
            <div className="flex items-center justify-center gap-3">
              <span className={`text-sm font-mono ${isDarkMode ? 'text-stone-500' : 'text-stone-400'}`}>ID:</span>
              <span className="text-sm font-black tracking-wide text-emerald-500 break-all">{roomCode}</span>
        ...
        <div className={`p-6 rounded-3xl border ${isDarkMode ? 'bg-black/40 border-white/5' : 'bg-white border-black/5'} backdrop-blur-xl shadow-2xl`}>
          ...
          <div className="space-y-4">
            {[
              ...
              ...opponents.map((op) => ({ ...op, isUser: false }))
