...
export default function App() {
  ...
  // Simular llegada de jugadores en el LOBBY LOCAL SOLAMENTE
  useEffect(() => {
    if (gameState === 'lobby' && currentPlayers < playerCount && !isOnline) {
      const timer = setTimeout(() => {
        setCurrentPlayers(prev => prev + 1);
      }, 2000 + Math.random() * 3000);
      return () => clearTimeout(timer);
    }
  }, [gameState, currentPlayers, playerCount, isOnline]);
  ...
  useEffect(() => {
    ...
    const handleGameState = (state: any) => {
      ...
      } else if (gameState === 'playing' && state.hasStarted) {
        // Actualizar tiles del servidor durante el juego
  ...
  const handleTileClick = (id: number) => {
    ...
    if (selectedId === null) {
      ...
      if (areMatching(firstTile, tile)) {
        ...
        if (isOnline) {
          socketService.submitTileMatch([id, selectedId]);
        }
      } else {
        // No match
        setCombo(0);
